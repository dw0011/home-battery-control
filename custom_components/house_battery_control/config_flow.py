...
from .const import (
    ...
    CONF_EXPORT_PRIORITY_THRESHOLD,
    ...
    DEFAULT_EXPORT_PRIORITY_THRESHOLD,
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Optional(CONF_USE_FLOW_POWER, default=DEFAULT_USE_FLOW_POWER): BooleanSelector(),
...
class HBCOptionsFlowHandler(config_entries.OptionsFlow):
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Optional(
                        CONF_USE_FLOW_POWER, default=self._data.get(CONF_USE_FLOW_POWER, DEFAULT_USE_FLOW_POWER)
                    ): BooleanSelector(),
                    vol.Optional(
    ...
    async def async_step_control(
        ...
        if user_input is not None:
            ...
            # Persist export priority threshold (0 = disabled)
            self._data[CONF_EXPORT_PRIORITY_THRESHOLD] = float(
                user_input.pop(CONF_EXPORT_PRIORITY_THRESHOLD, DEFAULT_EXPORT_PRIORITY_THRESHOLD) or 0.0
            )
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Optional(
                        CONF_EXPORT_PRIORITY_THRESHOLD,
                        description={
                            "suggested_value": self._data.get(CONF_EXPORT_PRIORITY_THRESHOLD, DEFAULT_EXPORT_PRIORITY_THRESHOLD)
                        },
                    ): NumberSelector(NumberSelectorConfig(
                        min=0, max=9999, step=0.5, unit_of_measurement="c/kWh",
                        mode=NumberSelectorMode.BOX,
