...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    tracker = TelemetryCostTracker(hass, config_data)
    await tracker.async_load()

    coordinator = HBCDataUpdateCoordinator(hass, entry.entry_id, config_data, tracker)
    ...
    try:
        ...
        async_register_built_in_panel(
            ...
            config={
                "_panel_custom": {
                    "name": "hbc-panel",
                    "module_url": "/hbc/frontend/hbc-panel.js?v=59",
                }
            },
        )
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    tracker = hass.data[DOMAIN].get(entry.entry_id, {}).get("tracker")
    if tracker:
